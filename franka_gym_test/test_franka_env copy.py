...
class FrankaEnv:
    def __init__(self):
        ...
        #self.env = gym.make('FrankaDrawerEnv_2', num_obst=3, n_substeps=20, control_mode="ik_controller", scenario=scenarios['lift_maze'])
        self.env.reset()
    ...
    def get_ee_pos(self):
        return self.env.get_ee_position()
    
    # get the information of the obstacles
    def get_obstacle_info(self):
        return self.env.get_obstacle_info()
 
def test_default_env():
    register_custom_envs()
    control_mode = ''
    env = gym.make('HumanoidStandup-v2', max_episode_steps=100,render_mode="human")
    obs, info = env.reset()
    for j in range(50):
        env.reset()
        for i in range(1000):
